...
function Appointments() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'linear-gradient(135deg, #f0f4f8, #e8f4fd)'}}>
      <div style={{background:'linear-gradient(135deg, #1a5276, #2d7dd2)',padding:'20px 30px',display:'flex',justifyContent:'space-between',alignItems:'center',boxShadow:'0 4px 15px rgba(0,0,0,0.2)'}}>
      ...
      <div style={{display:'flex',gap:'20px',padding:'20px',flexWrap:'wrap',maxWidth:'1100px',margin:'0 auto'}}>
        <div style={{background:'white',padding:'25px',borderRadius:'16px',boxShadow:'0 8px 25px rgba(0,0,0,0.1)',flex:1,minWidth:'300px',borderTop:'4px solid #2ecc71'}}>
          ...
          {message && <div style={{background:'#d5f5e3',color:'#1e8449',padding:'10px',borderRadius:'8px',marginBottom:'15px'}}>{message}</div>}
        ...
        <div style={{background:'white',padding:'25px',borderRadius:'16px',boxShadow:'0 8px 25px rgba(0,0,0,0.1)',flex:1,minWidth:'300px',borderTop:'4px solid #3498db'}}>
          ...
          {appointments.map((apt, index) => (
            <div key={index} style={{background:'linear-gradient(135deg, #f0f4f8, #e8f4fd)',padding:'15px',borderRadius:'12px',marginTop:'15px',borderLeft:'4px solid #2d7dd2'}}>
              ...
              <p style={{margin:'5px 0',color:'#2ecc71'}}><strong>Status:</strong> {apt.status} ✅</p>
            </div>
          ))}
        </div>
      </div>
